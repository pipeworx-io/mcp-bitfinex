...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Bitfinex v2 public');
}
...
const tools: McpToolExport['tools'] = [
  { name: 'tickers', description: 'Bitfinex crypto exchange — multi-symbol live tickers for crypto pairs. Pass comma-separated symbols like "tBTCUSD,tETHUSD" or "ALL". Returns bid/ask, last, daily change, volume per pair.', inputSchema: { type: 'object', properties: { symbols: { type: 'string' } }, required: ['symbols'] } },
  { name: 'ticker', description: 'Bitfinex crypto exchange — single-pair live ticker (e.g. "tBTCUSD"). Returns bid/ask, last trade price, 24h volume + change percentage.', inputSchema: { type: 'object', properties: { symbol: { type: 'string' } }, required: ['symbol'] } },
  {
    name: 'ticker_history',
    description: 'Bitfinex crypto exchange — historical ticker snapshots for one or more pairs (comma-separated symbols like \'tBTCUSD\'). Optional start/end (ms epoch) and limit. Returns bid/ask/last per timestamp.',
  ...
  {
    name: 'trades',
    description: 'Bitfinex crypto exchange — recent trade tape for a pair (e.g. \'tBTCUSD\'): trade ID, timestamp, amount, and price per execution. Optional start/end (ms epoch), limit, and sort order.',
  ...
  {
    name: 'book',
    description: 'Bitfinex crypto exchange order book (bids + asks) for a crypto pair like "tBTCUSD". Returns price levels with size + count. Use for live depth-of-book analysis, spread, market microstructure.',
  ...
  {
    name: 'candles',
    description: 'Bitfinex crypto OHLC candles for a crypto pair. Timeframes 1m through 1M. Returns timestamped open/high/low/close + volume. Use for charting, technical analysis, backtesting on Bitfinex.',
    inputSchema: {
      type: 'object',
  ...
  {
    name: 'liquidations',
    description: 'Bitfinex crypto exchange — the historical feed of forced margin liquidations across Bitfinex pairs (/v2/liquidations/hist). Each row carries the pair symbol, event timestamp, liquidated position size and the price it was closed at; optional start/end (ms epoch), limit and sort. Answers which leveraged crypto positions got liquidated on Bitfinex, when, and at what price.',
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  const get = async (path: string, params?: Record<string, unknown>) => {
    ...
    if (!res.ok) throw await httpError(res, 'Bitfinex');
    return res.json();
  };
